...
export default function Register() {
  ...
  { value: "Alergologie și imunologie clinică", label: "Allergology and Clinical Immunology" },
  { value: "Anatomie patologică", label: "Pathological Anatomy" },
  { value: "Anestezie și terapie intensivă", label: "Anesthesia and Intensive Care" },
  ...
  { value: "Chirurgie plastică, estetică și reconstructivă", label: "Plastic, Aesthetic and Reconstructive Surgery" },
  { value: "Chirurgie toracică", label: "Thoracic Surgery" },
  { value: "Chirurgie vasculară", label: "Vascular Surgery" },
  { value: "Dermatologie și venerologie", label: "Dermatology and Venereology" },
  { value: "Diabet, nutriție și boli metabolice", label: "Diabetes, Nutrition and Metabolic Diseases" },
  ...
  { value: "Gastroenterologie și hepatologie", label: "Gastroenterology and Hepatology" },
  ...
  { value: "Infecțioase și medicină tropicală", label: "Infectious and Tropical Medicine" },
  ...
  { value: "Medicina fizică și de reabilitare", label: "Physical Medicine and Rehabilitation" },
  ...
  { value: "Ortopedie și traumatologie", label: "Orthopedics and Traumatology"},
  ...
  { value: "Radiologie și imagistică medicală", label: "Radiology and Medical Imaging"},
  ...
  const verifyRegMed = (first, last, specialization, county) => {
    axios
  .get("https://regmed.cmr.ro/api/v2/public/medic/cautare/" + first + last)
  .then((response) => {
    ...
    if (results.length === 0)
      ...
      for(var i = 0; i < results.length; i++){
       if (county === results[i].judet)
       {
        var ok = 0;
        const specialitati = results[i].specialitati;
     
        for(var j = 0; j < specialitati.length; j++ ){
            if( specialization.toLowerCase() ===  specialitati[j].nume.toLowerCase())
              ok = 1;
        }
        if (ok === 1)
          message = "Found.";
    ...
    if (message === "")
      message = "Doctor is from wrong county or has wrong specialization.";
    if (message === "Found.")
      message = "";
  ...
  const verifyingInputs = (user) => {
    ...
    if (!ok) 
      ...
      verifyRegMed(user.firstName, user.lastName, user.specialization, user.county);
      ...
      >
        ...
        <div style={title}> Parkinson's </div>
          ...
          <Select
            ...
            onChange={(e) =>
              setUser({ ...user, specialization: e.value })
            }
          />
        </div>
        <div style={{ marginTop: "20px" }}>
